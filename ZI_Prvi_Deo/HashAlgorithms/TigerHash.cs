...
namespace ZI_Prvi_Deo.CryptoStrategies.HashAlgorithms
{
    public class TigerHash
    {
        ...
        public byte[] Hash(byte[] bytesToHash)
        {
            byte[] stuffedBytesToHash = ByteStuffer.FillMissingBytes(bytesToHash, 64, new byte[] { 0 });
            if (BitConverter.IsLittleEndian)
                Array.Reverse(stuffedBytesToHash);
           
            ulong h0 = 0x0123456789ABCDEF;
            ulong h1 = 0xFEDCBA9876543210;
            ulong h2 = 0xF096A5B4C3D2E187;

            for (int i = 0; i < stuffedBytesToHash.Length; i += 64)
            {
                ulong a = h0;
                ulong b = h1;
                ulong c = h2;
                ...
                for (int j = 0; j < 4; j++)
                {
                    for (int k = 0; k < 8; k++)
                    {
                        c ^= stuffedBytesToHash[i + k];
                        byte[] cBytes = BitConverter.GetBytes(c);
                        a -= s[cBytes[0]] ^ s[256 + cBytes[2]] ^ s[512 + cBytes[4]] ^ s[768 + cBytes[6]];
                        b += s[768 + cBytes[1]] ^ s[512 + cBytes[3]] ^ s[256 + cBytes[5]] ^ s[cBytes[7]];
                        b *= (ulong)(j + 1);
                    }

                    if (j == 0)
                    {
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i) - (BitConverter.ToUInt64(stuffedBytesToHash, i + 7)
                                                         ^ 0xA5A5A5A5A5A5A5A5)),
                                   0,
                                   stuffedBytesToHash,
                                   i,
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 8)
                                                         ^ BitConverter.ToUInt64(stuffedBytesToHash, i)),
                                   0,
                                   stuffedBytesToHash,
                                   i + 8,
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 16)
                                                         + BitConverter.ToUInt64(stuffedBytesToHash, i + 8)),
                                   0,
                                   stuffedBytesToHash,
                                   i + 16,
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 24)
                                                         - (BitConverter.ToUInt64(stuffedBytesToHash, i + 16)
                                                            ^ ((BitConverter.ToUInt64(stuffedBytesToHash, i + 8) ^ 0xFFFFFFFFFFFFFFFF) << 19))),
                                   0,
                                   stuffedBytesToHash,
                                   i + 24,
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 32)
                                                         ^ BitConverter.ToUInt64(stuffedBytesToHash, i + 24)),
                                   0,
                                   stuffedBytesToHash,
                                   i + 32,
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 40)
                                                         + BitConverter.ToUInt64(stuffedBytesToHash, i + 32)),
                                   0,
                                   stuffedBytesToHash,
                                   i + 40,
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 48)
                                                         - (BitConverter.ToUInt64(stuffedBytesToHash, i + 40)
                                                            ^ ((BitConverter.ToUInt64(stuffedBytesToHash, i + 32) ^ 0xFFFFFFFFFFFFFFFF) >> 23))),
                                   0,
                                   stuffedBytesToHash,
                                   i + 48,
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 56)
                                                         ^ BitConverter.ToUInt64(stuffedBytesToHash, i + 48)),
                                   0,
                                   stuffedBytesToHash,
                                   i + 56,
                                   8);
                    ...
                    {
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i)
                                                         + BitConverter.ToUInt64(stuffedBytesToHash, i + 56)),
                                   0,
                                   stuffedBytesToHash,
                                   i,
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 8)
                                                         - (BitConverter.ToUInt64(stuffedBytesToHash, i)
                                                            ^ ((BitConverter.ToUInt64(stuffedBytesToHash, i) ^ 0xFFFFFFFFFFFFFFFF) << 19))),
                                   0,
                                   stuffedBytesToHash,
                                   i + 8,
                                   8);
                        ...
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 32)
                                                         - (BitConverter.ToUInt64(stuffedBytesToHash, i + 24)
                                                            ^ ((BitConverter.ToUInt64(stuffedBytesToHash, i + 16) ^ 0xFFFFFFFFFFFFFFFF) >> 23))),
                                   0,
                                   stuffedBytesToHash,
                                   i + 32,
                                   8);
                        ...
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 48)
                                   ...
                                   8);
                        Array.Copy(BitConverter.GetBytes(BitConverter.ToUInt64(stuffedBytesToHash, i + 56)
                                                         - (BitConverter.ToUInt64(stuffedBytesToHash, i + 48) ^ 0x0123456789ABCDEF)),
                                   0,
                                   stuffedBytesToHash,
                                   i + 56,
                                   8);
            ...
            byte[] hashedBytes = new byte[24];
            Array.Copy(BitConverter.GetBytes(h0), hashedBytes, 8);
            Array.Copy(BitConverter.GetBytes(h1),0 , hashedBytes, 8, 8);
            Array.Copy(BitConverter.GetBytes(h2),0 , hashedBytes, 16, 8);
